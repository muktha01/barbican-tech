...
const GumSupplier = () => {
  ...
  const handleSubmit = async () => {
    if (!formData.supplierName || !formData.companyName || !formData.type) {
    ...
    try {
      ...
      if (editId) {
        response = await axios.patch(
          `http://localhost:8000/api/suppliers/${editId}`,
          payload
        );
        successMessage = "Supplier updated successfully!";
      } else {
        response = await axios.post("http://localhost:8000/api/suppliers", payload);
  ...
  const handleDeleteConfirm = async () => {
    ...
    setLoading(true);
    try {
      await axios.delete(`http://localhost:8000/api/suppliers/${deletingSupplier.id}`);
      fetchSuppliers();
...
<div className="fixed inset-0 bg-opacity-40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
 ...
 <form className="space-y-4 pt-4 overflow-y-auto" onSubmit={(e) => e.preventDefault()}>              <div>
